...
--time:ease-in-out
		...
		margin-right: 5px;}
`)
    document.body.insertAdjacentHTML("beforeend", `<div class="wan-box" style="transition: 0s !important;">
<label for="range">
页面宽度
<input type="range" min="10" max="100" value="64" id="range">
<span id="rangeNum">64</span><span id="unit">%视口宽度</span>
</label>
...
<span class="sliderBut"></span>
</label>
</div>
<label for="radiusInp" title="设置圆角大小单位像素">
圆角大小
<input type="range" min="0" max="25" value="5" id="radiusInp">
<span id="radiusNum">5</span>px
</label>
</div>
<div class="switch-container">
<div class="flex">
...
<label for="transitionInp" title="设置过渡时长单位秒">
过渡时长
<input type="range" min="0" max="5" step="0.1" value="0.3" id="transitionInp">
<span id="transitionNum">0.3</span>s
<div>
过渡曲线
<label for="ease-in-out" class="radio-mar">
快=>慢=>快
<input type="radio" name="time" data-name="ease-in-out" id="ease-in-out" checked>
</label>
<label for="ease-out" class="radio-mar">
慢=>快
<input type="radio" name="time" data-name="ease-out" id="ease-out">
</label>
<label for="linear" class="radio-mar">
匀速
<input type="radio" name="time" data-name="linear" id="linear">
</label>
</div>
</label>
</div>
<div class="wan-box-footer">
<button class="button" id="ok">确定</button>
<button class="button" id="no">取消</button>
<button class="button" id="reset" title="重置为Steam默认宽度">重置</button>
    ...
    /**
     * 修改指定DOM元素的宽度属性。
     * @param {string} selector - CSS选择器字符串，用于匹配要修改的元素。
     * @param {string} width - 设置的宽度值。
     * @param {string} [minWidth="auto"] - 设置的最小宽度值。
     * @param {string} [maxWidth="initial"] - 设置的最大宽度值。
     */
    function modifyWidth(selector, width, minWidth = "initial", maxWidth = "initial") {
     ...
     * 将指定CSS属性恢复到默认状态。
     * @param {string} selector - CSS选择器字符串，用于匹配要修改的元素。
     * @param {string} cssAttribute - 要恢复默认值的CSS属性名称。
     * @param {string} [state="initial"] - CSS属性的新值，默认为"initial"。
     */
    function resetStyleProperty(selector, cssAttribute, state = "initial") {
        const elements = document.querySelectorAll(selector);
        if (!elements || elements.length === 0) return;

        elements.forEach(element => {
            // 使用直接设置style对象的方式代替setAttribute以确保只修改单个属性
            element.style[cssAttribute] = state;
        });
    }

    // 整个页面
    modifyWidth("body.v6 .page_content", "var(--mainWidth)");
    // 导航栏
    modifyWidth("div#store_header .content", "var(--mainWidth)");
    // 轮播图部分
    modifyWidth("div.leftcol", "70%");
    // 轮播图右侧
    modifyWidth("div.rightcol", "30%");
    // 轮播图右侧游戏图片重置
    modifyWidth("img.game_header_image_full", "100%")
    // 头部下方查看队列
    modifyWidth(".queue_ctn", "var(--mainWidth)");
    // 安装Steam/马上开玩选项卡
    modifyWidth(".game_area_play_stats", "var(--mainWidth)");
    // 左侧购买游戏
    modifyWidth(".leftcol.game_description_column", "70%");
    // 右侧介绍
    modifyWidth(".rightcol.game_meta_data", "calc(30% - 14px)");
    // 在库中测评部分
    modifyWidth("#review_create", "100%");
    // 注意提示框
    modifyWidth(".notice_box_content", "calc(100% - 29px)");
    // 评论输入框
    modifyWidth("#game_recommendation", "calc(100% - 27px)");
    // 评论输入框下方
    modifyWidth(".review_controls", "calc(100% - 19px)");
    //语言
    modifyWidth(".game_language_options", "100%");
    // 游戏名称部分重置最大宽度
    resetStyleProperty(".apphub_HomeHeaderContent", "max-width")
    //轮播图右侧溢出换行
    resetStyleProperty(".glance_tags.popular_tags", "display", "flex");
    resetStyleProperty(".glance_tags.popular_tags", "flex-wrap", "wrap");
    resetStyleProperty(".glance_tags.popular_tags", "height", "fit-content");
    resetStyleProperty(".similar_recent_apps_container", "height", "fit-content");
    resetStyleProperty(".similar_recent_apps_container", "flex-wrap", "wrap");
    // 在库中测评部分
    resetStyleProperty("#review_create", "max-width")
    // 轮播图右侧游戏图片重置
    resetStyleProperty("#gameHeaderImageCtn img", "height")
    resetStyleProperty("#gameHeaderImageCtn img", "width", "100%")
    // 评论输入框
    resetStyleProperty("#game_recommendation", "max-width", "calc(100% - 27px)");
    // 评论输入框下方
    resetStyleProperty(".review_controls", "max-width", "calc(100% - 19px)");
